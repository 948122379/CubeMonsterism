...
#pragma fragment frag
		...
		//application to vertex
			...
			fixed4 frag(v2f f) :SV_Target{
				...
				fixed3 diffuse = _LightColor0.rgb*_Color.rgb*(dot(normalDir, lightDir)*0.5 + 0.5);
				...
				fixed3 specular = _Specular.rgb*pow(max(dot(normalDir, halfDir), 0), _Gloss);

				fixed3 tempColor = diffuse;// + specular;

				return fixed4(tempColor, _Color.a+saturate(dot(viewDir,normalDir)));
			}
